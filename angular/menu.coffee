window.MenuCtrl = ($scope, $routeParams, $http) ->
  ...
  $http.get('/api/menu?access_token=' + accessToken).success (data) ->
    $scope.data = data[0]
    return
...
#  data.save()
  ...
  $scope.formatCurrency = (num, prefix) ->
    num = if (isNaN(num) || num == '' || num == null) then 0.00 else num
    prefix + parseFloat(num).toFixed(2)
